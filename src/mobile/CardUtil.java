 ...
 * time on 2019/6/28  17:03
 */
public class CardUtil {
    static Map<String, MobileCard> cards = new HashMap<>();//注册新卡
    Map<String, List<ConsumInfo>> consumInfos = new HashMap<>();//话费充值
    List<Scene> scenes = new ArrayList<Scene>();


    public void initScene() {//用例场景

        MobileCard card1 = new MobileCard("阿飞", "123", "13900000000",
                new TalkPackage(), 58.0, 42.0);
        MobileCard card2 = new MobileCard("阿球", "123", "13900000001",
                new NetPackage(), 68.0, 32.0);
        MobileCard card3 = new MobileCard("阿挺", "123", "13900000002",
                new SuperPackage(), 78.0, 22.0);
        MobileCard card4 = new MobileCard("阿昊", "123", "13900000003",
    ...
    public void initScenes(){
        scenes.add(new Scene("通话",90,"问候客户，谁知其如此难缠 通话90分钟",10));
        scenes.add(new Scene("通话",30,"询问妈妈身体状况 本地通话30分钟",4));
        scenes.add(new Scene("短信",5,"参与环境保护实施方案问卷调查 发送短信5条",1));
        scenes.add(new Scene("短信",50,"通知朋友手机换号，发送短信50条",3));
        scenes.add(new Scene("上网",1*1024,"和女友微信视频聊天   使用流量1G",0));
        scenes.add(new Scene("上网",2*1024,"晚上手机在线看韩剧，不留神睡着啦！ 使用流量 2G",0));
    }

    public boolean isExistCard(String number, String passWord) {//判断号码是否是正确
        Set<String> num = cards.keySet();
        Iterator<String> it = num.iterator();
        while (it.hasNext()) {
            String SearchNum = it.next();
            if (SearchNum.equals(number) && cards.get(SearchNum).getPassword().equals(passWord)) {//判断输入的号码和密码是否与注册的密码一致
                return true;
            }
        }
        return false;
     ...
     */
    public boolean isExistCard(String SearchNumber) {//判断是否注册
        Set<String> numbers = cards.keySet();//创建一个set集合，元素为cards中的键值（电话号码）
     ...
     * @param card//创建新卡
     */
    public void addCard(MobileCard card) {//添加注册卡
    ...
    public void showRemainDetail(String searchNumber) {
        ...
        if (pack instanceof TalkPackage) {
            ...
            meg.append("上网流量：" + dataFormat(remainFlow * 1.0 / 1024)
                    + "GB");
    ...
    public void showAmountDetail(String searchNumber) {
        ...
        meg.append("合计：" + dataFormat(card.getConsumAmount()) + "元\n");
        meg.append("账户余额：" + dataFormat(card.getMoney()) + "元");
        // 显示本月消费详细信息
        System.out.println(meg);
    }
    ...
    //    public void changingPack(String number, String packNum) {//更换包装
//
//    }
    ...
    public void chargeMoney(String number, double money) {
        ...
        System.out.println("充值成功，当前话费余额为" + dataFormat(card.getMoney()) + "元。");
    }
